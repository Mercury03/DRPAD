...
os.makedirs('data/WADI', exist_ok=True)

Normal_SWAT = pd.read_excel('data/SWaT/SWaT_Dataset_Normal_v1.xlsx', header=1)
...
CSV_Normal_SWAT.to_csv('data/SWaT/Normal.csv', index=False, header=False)

Attack_SWAT = pd.read_excel('data/SWaT/SWaT_Dataset_Attack_v0.xlsx', header=1)
...
CSV_Attack_SWAT.to_csv('data/SWaT/Attack.csv', index=False, header=False)

# PSM: PSM does not need any change due to no discrete variate

# WADI
Normal_WADI = pd.read_csv('data/WADI/WADI_14days_new.csv', header=0)
...
CSV_Normal_WADI.to_csv('data/WADI/Normal.csv', index=False, header=False)

Attack_WADI = pd.read_csv('data/WADI/WADI_attackdataLABLE.csv', header=1)
...
CSV_Attack_WADI.to_csv('data/WADI/Attack.csv', index=False, header=False)
...
# MSDS
logs = pd.read_csv('data/MSDS/concurrent_data/logs/logs_aggregated_concurrent.csv')
...
file_path = 'data/MSDS/concurrent_data/metrics/'
...
np.savetxt('data/MSDS/labels.csv', labels, fmt='%.f', delimiter=',')
np.savetxt('data/MSDS/train.csv', train_data, fmt='%.6f', delimiter=',')
np.savetxt('data/MSDS/test.csv', test_data, fmt='%.6f', delimiter=',')


# pruned and remedied MSL
values = pd.read_csv('data/MSL/labeled_anomalies.csv')
values = values[values['spacecraft'] == 'MSL']
filenames = values['chan_id'].values.tolist()
test = np.load(f'data/MSL/test/C-1.npy')
...
values = pd.read_csv('data/SMAP/labeled_anomalies.csv')
values = values[values['spacecraft'] == 'SMAP']
filenames = values['chan_id'].values.tolist()
test = np.load(f'data/SMAP/test/P-1.npy')
labels = np.zeros(test.shape[0])
